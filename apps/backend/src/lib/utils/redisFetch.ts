...
async function redisFetchMultipleToServer(
	action: string,
	...
	const transaction = await redis.mget(ifTransactionExist);
	const parsedTransaction = transaction.map((ele: any) => {
		return JSON.parse(ele as string).request;
	});
...
async function findIncompleteOnConfirmCalls() {
	try {
		...
		const incompleteCalls = values
			...
			.filter(
				(item) =>
					(item.request.context.domain === SERVICES_DOMAINS.AGRI_SERVICES ||
						item.request.context.domain ===
							SERVICES_DOMAINS.HEALTHCARE_SERVICES) &&
					new Date(item.response.timestamp) > new Date(fiveMinutesAgo)
			);
